 ...
 */
function dispensary_blocks_render_block_core_latest_topicals( $attributes ) {
		...
		foreach ( $recent_products as $product ) {
			...
			$list_items_markup .= sprintf(
				...
				$list_items_markup .= sprintf(
					...
					$product_details = apply_filters( 'wpd_topicals_block_product_details', $product_details );
 ...
 */
function dispensary_blocks_register_block_core_latest_topicals() {
	...
	register_block_type( 'dispensary-blocks/wpd-topicals-grid', array(
		'attributes' => array(
			'categories' => array(
				'type' => 'string',
			),
		...
		'render_callback' => 'dispensary_blocks_render_block_core_latest_topicals',
	) );
}

add_action( 'init', 'dispensary_blocks_register_block_core_latest_topicals' );

/**
 * Create API fields for additional info
 */
function dispensary_blocks_register_topicals_rest_fields() {
	// Add landscape featured image source
	register_rest_field(
		'post',
		'featured_image_src',
		array(
			'get_callback'    => 'dispensary_blocks_get_topicals_image_src_landscape',
	...
	register_rest_field(
		...
		array(
			'get_callback'    => 'dispensary_blocks_get_topicals_image_src_square',
			'update_callback' => null,
			'schema'          => null,
		)
	);
...
add_action( 'rest_api_init', 'dispensary_blocks_register_topicals_rest_fields' );


/**
 * Get landscape featured image source for the rest field
 */
function dispensary_blocks_get_topicals_image_src_landscape( $object, $field_name, $request ) {
 ...
 */
function dispensary_blocks_get_topicals_image_src_square( $object, $field_name, $request ) {
	$feat_img_array = wp_get_attachment_image_src(
		$object['featured_media'],
		'wpd-small',
		false
